...
# After obtaining the AST, we can traverse the tree node one by one to generate the
# IR. The IR builder is inside `allo/ir/builder.py`. Basically, the builder is a
# dispatcher that maps the AST node to the corresponding IR builder function, i.e.,
# the `FunctionDef` node will be mapped to `ASTTransformer.build_FunctionDef`.
...
class ASTTransformer(ASTBuilder):
    # All the builder function are staticmethods that take in two arguments: an AST
    # context and an AST node. The AST context stores necessary information used to
    # build the IR, including:
    #     `ip_stack`: The stack of insertion points. The insertion point is used to
    #                 denote the current position of the IR builder, i.e., when we
    #                 are building the body of a function, the insertion point is
    #                 the function body.
    #     `buffers`: The dictionary that stores all the tensors in the program.
    #     `induction_vars`: The list of loop iterators, e.g., i, j, k.
    #     `global_vars`: The global variables defined outside the user-defined func-
    #                    tion.
    ...
    # The `For` node is the for-loop statement. We provide different APIs to support
    # different loop structures, so we need to further dispatch the `For` node to the
    # corresponding builder function. For example, here we use `allo.grid`, so it will
    # be dispatched to `build_grid_for`.
    # We provide some helper functions in `allo/ir/transform.py` to make the IR cre-
    # ation easier. In this case, we can just call `build_for_loops` and pass in the
    # bounds and the names of the loops to create a loop nest. Before building the
    # loop body, we need to update the insertion point:
    # ```python
    # ctx.set_ip(for_loops[-1].body.operations[0])
    # ```
    # After calling `build_stmts(ctx, node.body)`, we also need to recover the inser-
    # tion point: `ctx.pop_ip()`.
    @staticmethod
    ...
    # The `AnnAssign` node is the annotated assignment statement. The `build_AnnAssign`
    # function first evaluate the right hand side of the assignment statement by call-
    # ing `rhs = build_stmt(ctx, node.value)`. Then, it gets the user-defined type an-
    # notation to generate correct data types for the tensor. Please refer to `memref`
    # dialect for more details. Similarly, we can call `memref_d.AllocOp` to create a
    # new memory allocation, and you can see the actual `memref.alloc` operation in
    # the generated MLIR code. One more thing to mention is that what we see inside the
    # AST is just string, so if we want to get the actual value of a literal, we need
    # to retrieve it from the `ctx.global_vars` dictionary. For example, `int32[M, N]`
    ...
    #       ),
    #     )
    # We can see the `M` and `N` are just the `Name` nodes, and we need to retrieve
    # the actual value from the `ctx.global_vars` dictionary by calling something
    # like `ctx.global_vars[node.slice.value.elts[0].id]`.
    @staticmethod
    ...
    # The `FunctionDef` node is the function definition. The `build_FunctionDef`
    # function first creates the input and output data types based on users' an-
    # notations. Then, it creates a new MLIR function operation by calling
    # ```python
    #     func_op = func_d.FuncOp(name=node.name, type=func_type, ip=ip, loc=loc)
    # ```
    # Here, `func_d` is the `func` dialect defined in MLIR. The `FuncOp` is the
    # operation that represents a function in MLIR. The function arguments are 
    # explained below:
    #     `name`: the name of the function, we directly use the AST `FunctionDef`
    #             node's name as the operation name.
    #     `type`: the `FunctionType` that defines the input and output types of
    #             the function.
    #     `ip`: the insertion point of the function, which is the current inser-
    #           tion point of the AST context, and we can directly obtain it by
    #           calling ctx.get_ip().
    #     `loc`: the actual line number of the function, which can be usually
    #            omitted.
    # After creating the function operation, we need to create the function body.
    # We first update the insertion point to the function body by calling `ctx.
    # set_ip(func_op.entry_block)`. Then, we traverse the function body and re-
    # cursively call build_stmt. The function arguments are inserted into the 
    # buffers for further usage.
    @staticmethod
    ...
    # We can see the `build_Module` function only does one thing: traverse the sta-
    # tements inside the body of the module, and recursively call `build_stmt`.
    @staticmethod
